...
urlpatterns = [
                ...
                url(r'^location/(?P<species>[\w_]+)/(?P<seqtype>[\w]+)/$', method_router, {'GET': views.location_lookup_GET, 'POST': views.location_lookup_POST}, name='location_lookup'),
                url(r'^sequence/(?P<species>[\w_]+)/(?P<assembly>[\w_]+)/$', method_router, {'GET': views.sequence_GET, 'POST': views.sequence_POST}, name='sequence_fetcher'),
...
#                url(r'^diff/(?P<species>[\w]+)/(?P<release1>[\w\.]+)/(?P<release2>[\w\.]+)/$', views.transcript_diff),
                url(r'^diff/(?P<species>[\w]+)/(?P<release1>[\w\.]+)/(?P<release2>[\w\.]+)/$', views.release_diff),
